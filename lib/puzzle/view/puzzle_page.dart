      ...
      providers: [
        ...
        BlocProvider(
          create: (context) => ThemeBloc(
            initialThemes: [
              // const SimpleTheme(),
              context.read<CoidonThemeBloc>().state.theme, //====
              // (themes.toList()..shuffle()).last,
            ],
          ),
        ),
        BlocProvider(
...
class PuzzleView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const shufflePuzzle = true;

    return Scaffold(
      body: AnimatedContainer(
        duration: PuzzleThemeAnimationDuration.backgroundColorChange,
        decoration: BoxDecoration(color: theme.backgroundColor),
        child: BlocListener<CoidonThemeBloc, CoidonThemeState>(
          listener: (context, state) {
            final coidonTheme = context.read<CoidonThemeBloc>().state.theme;
            // final coidonTheme = (themes.toList()..shuffle()).first; //=====
            context.read<ThemeBloc>().add(ThemeUpdated(theme: coidonTheme));
          },
          child: MultiBlocProvider(
            providers: [
              ...
              BlocProvider(
                create: (context) => PuzzleBloc(4) //***** PUZZLE SIZE */
                  ..add(
