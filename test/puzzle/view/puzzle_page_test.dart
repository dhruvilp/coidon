...
void main() {
  group('PuzzlePage', () {
    ...
    testWidgets('provides all Dashatar themes to PuzzleView', (tester) async {
      ...
      final coidonThemes =
          puzzleViewContext.read<CoidonThemeBloc>().state.themes;
  ...
  group('PuzzleView', () {
    ...
    setUp(() {
      ...
      when(() => coidonThemeBloc.state)
          .thenReturn(CoidonThemeState(themes: [TigerCoidonTheme()]));
    ...
    testWidgets(
        ...
        'when CoidonTheme changes', (tester) async {
      final themes = [TigerCoidonTheme(), ElephantCoidonTheme()];

      whenListen(
        coidonThemeBloc,
        Stream.fromIterable([
          CoidonThemeState(themes: themes, theme: TigerCoidonTheme()),
          CoidonThemeState(themes: themes, theme: ElephantCoidonTheme()),
        ]),
      );
      ...
      verify(() => themeBloc.add(ThemeUpdated(theme: TigerCoidonTheme())))
          .called(1);

      verify(() => themeBloc.add(ThemeUpdated(theme: ElephantCoidonTheme())))
          .called(1);
    });
    ...
    group('PuzzleMenu', () {
      testWidgets(
        ...
        final themes = [SimpleTheme(), TigerCoidonTheme()];
      ...
      group('when tapped', () {
        ...
        testWidgets(
          ...
          await tester.pumpApp(
            PuzzleMenuItem(
              theme: TigerCoidonTheme(),
              themeIndex: themes.indexOf(TigerCoidonTheme()),
            ),
            themeBloc: themeBloc,
            puzzleBloc: puzzleBloc,
          );
